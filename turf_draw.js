// ─── Turf Draw Module ─────────────────────────────────────────────────────────
// Admin: draw new turf polygons (auto-populates houses from parcels.js),
//        edit existing boundaries (diffs houses), delete.
...
const TurfDraw = (() => {
  ...
  let _editLayer     = null;
  let _pendingRing   = null;   // ring from last draw, held while modal open
  ...
  function toggle() {
    ...
    if (_active) {
      ...
      UI.toast('Draw mode ON — draw a polygon to create a turf', 'info');
    } else {
  ...
  function _showDiffModal({ letter, layer, toKeep, toRemove, toAdd, excluded }) {
    ...
    UI._modal(`Edit Turf ${letter} Boundary`, `
      ...
      <div class="f-hint" style="margin-top:10px">Houses with recorded results are always preserved regardless of boundary.</div>
    `, () => {
      const geojson = layer.toGeoJSON().geometry;
  ...
  // ── Commercial toggle refreshes count ────────────────────────────────────
  ...
  function resortTurf(letter, startPt) {
    ...
    const sortedUnvisited = ParcelsUtil.walkOrder(withoutResult, pt || { lat: 32.972, lon: -96.978 });
    App.reorderTurfHouses(letter, [...withResult, ...sortedUnvisited]);
  ...
  return { init, toggle, isActive, loadTurfs, removeTurfLayer, startEditBoundary, resortTurf, _onCommercialToggle };
})();
